...
import { LogOut, Plus, Edit, Trash2, X, Image as ImageIcon, RotateCcw, Upload, Search, Percent, User, Lock, Save, Camera, Layout, CheckCircle, Loader2 } from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  // Define categories for the product form to fix the 'Cannot find name' error
  const categories = [
  ...
  const handleHeroImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        ...
        showToast("Imagen de Inicio actualizada.");
      } catch (err) {
  ...
  const handleAboutImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        const base64 = await processImage(e.target.files[0], 800, 1000);
        updateSettings({ aboutImage: base64 });
        showToast("Imagen de Fachada actualizada.");
      } catch (err) {
        alert("Error al procesar: " + err);
      } finally {
        setIsProcessingImage(null);
  ...
  const openModal = (item?: MenuItem) => {
    if (item) {
      ...
      setFormData({
        ...
        image: '', popular: false, discountPercent: ''
      });
      setPreviewImage('');
    }
    setIsModalOpen(true);
  ...
  const handleProfileImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      try {
        const base64 = await processImage(e.target.files[0], 400, 400);
  ...
  const handleProfileSubmit = (e: React.FormEvent) => {
    ...
    const result = updateAdminProfile({
      image: profileData.image, email: profileData.email,
    ...
    } else setProfileMessage({ type: 'error', text: result.message });
  };
    ...
    <div className="min-h-screen bg-black text-slate-200 p-4 md:p-8">
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex border-b border-slate-800 mb-8 overflow-x-auto no-scrollbar">
           <button onClick={() => setActiveTab('products')} className={`px-8 py-4 text-sm font-bold transition-all border-b-2 flex items-center gap-2 whitespace-nowrap ${activeTab === 'products' ? 'border-orange-600 text-orange-500 bg-orange-600/5' : 'border-transparent text-slate-500 hover:text-white'}`}><Search size={18} /> Gestión de Menú</button>
           <button onClick={() => setActiveTab('settings')} className={`px-8 py-4 text-sm font-bold transition-all border-b-2 flex items-center gap-2 whitespace-nowrap ${activeTab === 'settings' ? 'border-orange-600 text-orange-500 bg-orange-600/5' : 'border-transparent text-slate-500 hover:text-white'}`}><Layout size={18} /> Personalización Visual</button>
        </div>
          ...
          <>
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
              ...
              <div className="flex gap-2 w-full md:w-auto">
                <button onClick={resetMenu} className="bg-slate-800 hover:bg-slate-700 text-slate-400 p-3 rounded-xl border border-slate-700" title="Restaurar Menú"><RotateCcw size={20} /></button>
                <button onClick={() => openModal()} className="flex-1 md:flex-none bg-orange-600 hover:bg-orange-700 text-white px-6 py-3 rounded-xl flex items-center justify-center gap-2 font-bold transition-all shadow-lg shadow-orange-600/20"><Plus size={20} /> Nuevo Producto</button>
              </div>
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <thead className="bg-slate-950 text-slate-400 text-xs uppercase tracking-wider">
                    <tr><th className="p-5 font-semibold">Producto</th><th className="p-5 font-semibold">Precio</th><th className="p-5 font-semibold text-right">Acciones</th></tr>
                  </thead>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-4">
             <div className="bg-slate-900 p-8 rounded-2xl border border-slate-800 shadow-xl">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3"><div className="bg-orange-600/20 p-2 rounded-lg text-orange-500"><Layout size={24} /></div><h3 className="text-xl font-bold text-white">Imagen de Inicio (Hero)</h3></div>
                  {isProcessingImage === 'hero' && <Loader2 className="animate-spin text-orange-500" />}
                </div>
                <div className="relative aspect-video rounded-xl overflow-hidden border-2 border-slate-800 bg-slate-950 group">
                   <img src={settings.heroImage} className="w-full h-full object-cover transition-opacity duration-300" />
                   <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                      <button disabled={!!isProcessingImage} onClick={() => heroFileInputRef.current?.click()} className="bg-orange-600 text-white px-6 py-2 rounded-full font-bold flex items-center gap-2 hover:bg-orange-700 shadow-xl disabled:opacity-50"><Upload size={18} /> Cambiar Imagen</button>
                      <p className="text-[10px] text-slate-400 mt-2 uppercase tracking-widest font-bold">1920 x 1080 Recomendado</p>
                   </div>
                </div>
             ...
             <div className="bg-slate-900 p-8 rounded-2xl border border-slate-800 shadow-xl">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3"><div className="bg-orange-600/20 p-2 rounded-lg text-orange-500"><Camera size={24} /></div><h3 className="text-xl font-bold text-white">Foto de la Sede (Fachada)</h3></div>
                  {isProcessingImage === 'about' && <Loader2 className="animate-spin text-orange-500" />}
                </div>
                <div className="relative aspect-video rounded-xl overflow-hidden border-2 border-slate-800 bg-slate-950 group">
                   <img src={settings.aboutImage} className="w-full h-full object-cover transition-opacity duration-300" />
                   <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                      <button disabled={!!isProcessingImage} onClick={() => aboutFileInputRef.current?.click()} className="bg-orange-600 text-white px-6 py-2 rounded-full font-bold flex items-center gap-2 hover:bg-orange-700 shadow-xl disabled:opacity-50"><Upload size={18} /> Cambiar Fachada</button>
                      <p className="text-[10px] text-slate-400 mt-2 uppercase tracking-widest font-bold">800 x 1000 Recomendado</p>
                   </div>
                </div>
             ...
             <div className="lg:col-span-2 bg-orange-600/10 border border-orange-600/20 p-6 rounded-2xl flex items-center justify-between">
                <div><h4 className="text-orange-500 font-bold flex items-center gap-2"><Save size={18} /> Autoguardado activado</h4><p className="text-slate-400 text-sm">Al seleccionar una imagen, el sitio se actualiza automáticamente.</p></div>
             </div>
          </div>
        )}
      </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-5">
                  ...
                  <label className="flex items-center gap-3 text-sm text-slate-300"><input type="checkbox" checked={formData.popular} onChange={e => setFormData({...formData, popular: e.target.checked})} className="w-5 h-5" /> Producto Destacado</label>
                </div>
              </div>
